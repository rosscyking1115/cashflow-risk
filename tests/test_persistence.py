...
def test_production_refuses_to_start_without_a_database_url(
    ...
    """A missing DATABASE_URL in production must fail loudly, not quietly serve
    from a file the host discards on every redeploy. Silently degrading is how a
    documented guarantee becomes a false one without anyone deciding to change it."""
