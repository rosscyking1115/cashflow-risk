...
| R7 | Every score shows drivers; presented as a *ranking score*, not a calibrated probability (measured mean ECE 0.212 — see model-evaluation.md), so it cannot be read as odds about a named customer; rectification on request; human always in the loop | Built (explainable scores); calibration outstanding |
